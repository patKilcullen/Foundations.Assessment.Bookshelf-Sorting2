...
// searchSection.append(searchInput, searchButton)

searchButton.addEventListener('click', function(){
let searchValue = document.getElementById("searchDrop").value
let searchInput = document.getElementById("searchInput").value
if(searchValue === 'author'){
bookshelf1.searchArray =  bookshelf1.bookArray.filter(book=> book.author.find(string=>string.includes(searchInput)))
}if(searchValue === 'title'){
    bookshelf1.searchArray =   bookshelf1.bookArray.filter(book=> book.title === searchInput )  
}if(searchValue === 'subjects'){
    bookshelf1.searchArray =   bookshelf1.filter(book=> book.subject.find(string=>string.toLowerCase().includes(searchInput.toLowerCase()))) 
}
...
//  }
//  searchAuthor(name){
//      return this.bookArray.filter(book=> book.author.find(string=>string.includes(name)))
//  }
//  searchSubject(subject){
//      return this.bookArray.filter(book=> book.subject.find(string=>string.toLowerCase().includes(subject.toLowerCase())))
//  }
...
actionsSection.append(sortSection, favesSection, searchSection);
